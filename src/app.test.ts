...
//       const response = await request(appTest).get("/pokemon/search?name=missingno");

//       expect(JSON.parse(response.text)).toEqual([]);
//     });

//     it("sends the Pokemon when the name exactly matches", async () => {
//       const response = await request(appTest).get(`/pokemon/search?name=${pokemon[0].name}`);
...
//       const response = await request(appTest).get("/pokemon/9001");
...
//       const response = await request(appTest).get("/pokemon/123");
...
//       const response = await request(appTest).get("/run/runny/runner");
